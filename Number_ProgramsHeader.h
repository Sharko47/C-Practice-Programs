...
while(counter>0){//Findind the length of a number.
counter = counter/10;
++count;
}
printf("Number of Digits = %d\n",count);
int disarium_copy = disarium_number;
while(disarium_number>0){//Calculating the sum of digits powered with their respective position.
    rem = disarium_number%10;
    final_result = final_result + (int)powf(rem,count);
    disarium_number = disarium_number/10;
    count--;
}
if(final_result == disarium_copy){//Condition to check, if a final result is same as a received number then it's a Disarium Number.
printf("%d is a Disarium Number = %d\n",disarium_copy,final_result);
}else
{
    /* code */
...
int IsHappyNumber(int temp_num){      //Function of calculating the sum of Happy Number.
int remain,sum = 0;
while(temp_num>0){
    remain = temp_num%10;
...
int check_Pronic_Number(int number){//A Function to Check whether received number is Pronic Number or Not.
    int temp,i;
    for(i = 1;i<=number;i++){
        temp = i * (i-1);
        if(temp == number){
